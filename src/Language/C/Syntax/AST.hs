...
data CDeclarationItem a
  = CDeclarationItem
      (CDeclarator a) -- declarator (may be omitted)
      (Maybe (CInitializer a)) -- optional initialize
      (Maybe (CExpression a))
  | CDeclarationExpr (CExpression a)
  deriving (Eq, Show, Data, Typeable, Generic, Generic1, Functor {-! ,CNode ,Annotated !-})

pattern CDeclI a = CDeclarationItem a Nothing Nothing
...
partitionDeclSpecs = foldr deals ([], [], [], [], [], [])
 where
...
data CExpression a
  ...
  | CAlignofType
      ...
      a
  | CComplexReal
      (CExpression a) -- real part of complex number
      a
  | CComplexImag
      (CExpression a) -- imaginary part of complex number
      a
  | CIndex
      (CExpression a) -- array
      (CExpression a) -- index
  ...
  | CMember
      ...
      a
  | CVar
      (Identifier a) -- identifier (incl. enumeration const)
      a
  ...
  | -- | C11 generic selection
    CGenericSelection (CExpression a) [CGenericSelector a] a
  | -- | GNU C compound statement as expr
    CStatExpr (CStatement a) a
  | -- | GNU C address of label
    CLabAddrExpr (Identifier a) a
