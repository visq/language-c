...
prettyUsingInclude (CTranslUnit edecls _) =
 ...
 where
  ...
  tagIncludedDecls edecl
    | maybe False isHeaderFile (fileOfNode edecl) = Left ((posFile . posOf) edecl)
...
instance Pretty CStat where
  ...
  prettyPrec p (CCompound localLabels bis _) =
   ...
   where
    ppLblDecls = vcat . map (\l -> text "__label__" <+> identP l <+> semi)
...
instance Pretty CAsmStmt where
  pretty (CAsmStmt tyQual expr outOps inOps clobbers _) =
   ...
   where
    ...
    ops =
      text ":"
        <+> hcat (punctuate comma (map pretty outOps))
        <+> text ":"
        <+> hcat (punctuate comma (map pretty inOps))
...
instance Pretty CDecl where
  ...
  pretty (CDecl specs divs _) =
   ...
   where
    ...
    getAttrs (CDeclr _ _ _ cattrs _) = cattrs
...
instance Pretty CStructUnion where
  ...
  pretty (CStruct tag ident (Just decls) cattrs _) =
    vcat
      [ pretty tag <+> attrlistP cattrs <+> maybeP identP ident <+> text "{"
...
instance Pretty CEnum where
  ...
  pretty (CEnum enum_ident (Just vals) cattrs _) =
    vcat
...
instance Pretty CInit where
  ...
  pretty (CInitList (CInitializerList initl) _) =
    text "{" <+> hsep (punctuate comma (map p initl)) <+> text "}"
   where
...
instance Pretty CExpr where
  ...
  prettyPrec p (CBinary op expr1 expr2 _) =
    let prec = binPrec op
     in parenPrec p prec
          $ prettyPrec prec expr1
          <+> pretty op
          <+> prettyPrec (prec + 1) expr2
  ...
  prettyPrec p (CCall expr args _) =
    parenPrec p 30
      $ prettyPrec 30 expr
      <> text "("
      <> (sep . punctuate comma . map pretty) args
      <> text ")"
  ...
  prettyPrec _p (CCompoundLit decl (CInitializerList initl) _) =
    parens (pretty decl) <+> (braces . hsep . punctuate comma) (map p initl)
   where
    p ([], initializer) = pretty initializer
...
instance Pretty CBuiltin where
  ...
  pretty (CBuiltinOffsetOf _ty_name otherDesigs _) =
    error
      $ "Inconsistent AST: Cannot interpret designators in offsetOf: "
