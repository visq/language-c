...
import {-# SOURCE #-} Language.C.Analysis.AstAnalysis (ExprSide (..), tExpr)
...
import Control.Monad (ap, liftM, unless, when, zipWithM)
...
tParamDecl (CDecl declspecs declrs node) =
 ...
 where
  ...
  mkParamDecl name storage attrs ty declr_node =
    let vd = VarDecl name (DeclAttrs noFunctionAttrs storage attrs) ty
     in case name of
...
tMemberDecls (CDecl declspecs declrs node) = zipWithM tMemberDecl (True : repeat False) declrs
 where
  tMemberDecl handle_sue_def (CDeclarationItem member_declr Nothing bit_field_size_opt) =
    -- TODO: use analyseVarDecl here, not analyseVarDecl'
    do
      var_decl <- analyseVarDecl' handle_sue_def declspecs member_declr [] Nothing
      let (VarDeclInfo name fun_spec storage_spec attrs ty _node_info) = var_decl
      --
      checkValidMemberSpec fun_spec storage_spec
      return
        $ MemberDecl
          (VarDecl name (DeclAttrs noFunctionAttrs NoStorage attrs) ty)
          bit_field_size_opt
          node
  tMemberDecl handle_sue_def (CDeclarationExpr bit_field_size) =
    do
      let (storage_specs, _attrs, typequals, typespecs, _funspecs, _alignspecs) = partitionDeclSpecs declspecs
  ...
  checkValidMemberSpec fun_spec storage_spec =
    do
      when (fun_spec /= noFunctionAttrs) $ astError node "member declaration with inline specifier"
        ...
        do
          ...
          t <-
            tType
              True
              node
              (map CAttrQual (attrs ++ attrs_decl) ++ typequals)
              canonTySpecs
              derived_declrs
              []
...
tType handle_sue_def top_node typequals canonTySpecs derived_declrs oldstyle_params =
 ...
 where
  ...
  buildArrayType arr_quals size _node inner_ty =
    do
      (quals, attrs) <- tTypeQuals arr_quals
  ...
  case canonTySpec of
    ...
    TSNum tsnum -> do
      ...
      return
        . baseType
        $ case numType of
          Left (floatType, iscomplex)
            | iscomplex -> TyComplex floatType
...
tEnumType sue_ref enumerators attrs node = do
 ...
 where
  ...
  nextEnumerator memo (CEnumVar ident e) =
    let (memo', expr) = nextEnrExpr memo e
     in (memo', Enumerator ident expr ty (nodeInfo ident))
...
tNumType (NumTypeSpec basetype sgn sz iscomplex) =
  case (basetype, sgn, sz) of
    (BaseChar, _, NoSizeMod)
      | Signed <- sgn -> intType TySChar
    ...
    (intbase, _, NoSizeMod) | optBase BaseInt128 intbase ->
      intType $ case sgn of
        Unsigned -> TyUInt128
        _ -> TyInt128
    (intbase, signed, sizemod)
      | optBase BaseInt intbase
      , optSign Signed signed ->
          intType $ case sizemod of
            ShortMod -> TyShort
            LongMod -> TyLong
            LongLongMod -> TyLLong
...
tTypeQuals = foldrM go (noTypeQuals, [])
 where
...
canonicalTypeSpec = foldrM go TSNone
 where
...
canonicalStorageSpec storagespecs = liftM elideAuto $ foldrM updStorage NoStorageSpec storagespecs
 where
...
mergeOldStyle node oldstyle_params (CFunDeclr params attrs fdnode : dds) =
  case params of
    CFunParamsOld list -> do
      ...
      unless (Map.null param_map')
        $ astError node
        $ "declarations for parameter(s) "
        ++ showParamMap param_map'
 ...
 where
  ...
  implicitIntParam param_name =
    let nInfo = nodeInfo param_name
     in CDecl [CTypeSpec (CIntType nInfo)] [CDeclI (CDeclr (Just param_name) [] Nothing [] nInfo)] nInfo
...
splitCDecl decl@(CDecl declspecs declrs node) =
  case declrs of
    ...
    (d1 : ds) ->
      let declspecs' = map elideSUEDef declspecs
       in return $ (CDecl declspecs [d1] node) : [CDecl declspecs' [declr] node | declr <- ds]
 where
...
getOnlyDeclr (CDecl _ [CDeclarationItem declr _ _] _) = return declr
