...
analyseDecl is_local decl@(CDecl declspecs declrs node)
  | null declrs =
      case typedef_spec of
        Just _ -> astError node "bad typedef declaration: missing declarator"
        Nothing -> voidM $ analyseTypeDecl decl
  | (Just declspecs') <- typedef_spec = mapM_ (uncurry (analyseTyDef declspecs')) declr_list
  | otherwise = do
      let (storage_specs, attrs, typequals, typespecs, funspecs, _alignspecs) =
 ...
 where
  ...
  analyseVarDeclr specs handle_sue_def (CDeclarationItem declr init_opt Nothing) = do
    -- analyse the declarator
    ...
    if isFunctionType typ
      then extFunProto vardeclInfo
        ...
        (if is_local then localVarDecl else extVarDecl)
    ...
    return ()
  analyseVarDeclr _ _ (CDeclarationExpr _bitfieldSz) = astError node "bitfield size in object declaration"
...
analyseTypeDef handle_sue_def declspecs declr node_info = do
 ...
 where
  checkValidTypeDef fun_attrs _ _
    | fun_attrs /= noFunctionAttrs =
...
computeFunDefStorage ident other_spec = do
  ...
  case other_spec of
    ...
    bad_spec ->
      throwTravError
        $ badSpecifierError (nodeInfo ident)
        $ "unexpected function storage specifier (only static or extern is allowed)"
        ++ show bad_spec
...
extFunProto (VarDeclInfo var_name fun_spec storage_spec attrs ty node_info) =
  do
    when (isNoName var_name) $ astError node_info "NoName in extFunProto"
...
extVarDecl (VarDeclInfo var_name fun_spec storage_spec attrs typ node_info) init_opt =
  do
    when (isNoName var_name) $ astError node_info "NoName in extVarDecl"
 ...
 where
  ...
  globalStorage RegSpec =
    do
      when (isJust init_opt) $ astError node_info "initializer given for global register variable"
      case var_name of
  ...
  globalStorage (ExternSpec thread_local) =
    case init_opt of
      ...
      Just _ -> do
        warn $ badSpecifierError node_info "Both initializer and `extern` specifier given - treating as definition"
...
localVarDecl (VarDeclInfo var_name fun_attrs storage_spec attrs typ node_info) init_opt =
  do
    when (isNoName var_name) $ astError node_info "NoName in localVarDecl"
...
tStmt c (CFor i g inc s _) =
  do
    ...
    return voidType
 where
  checkExpr e = voidM $ tExpr c RValue e
...
tExpr' c side (CBinary op le re ni) =
  do
    when (side == LValue) $ typeError ni "binary operator as lvalue"
  ...
  case dropWhile (maybe True (not . typesMatch ty_sel) . fst) ty_list of
    ...
    [] -> case def_expr_ty of
      ...
      Nothing ->
        astError
          ni
          ( "no clause matches for generic selection (not fully supported) - selector type is "
              ++ show (pretty ty_sel)
              ++ ", available types are "
              ++ show (map (pretty . fromJust . fst) (filter (isJust . fst) ty_list))
          )
 where
  ...
  | identToString i == "__builtin_choose_expr" =
      case args of
        [g, e1, e2] ->
          ...
          do
            b <- constEval defaultMD Map.empty g
            case boolValue b of
